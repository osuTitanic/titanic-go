...
func (r *RelationshipRepository) ByUserAndTarget(userId int, targetId int) (*schemas.Relationship, error) {
	var relationship schemas.Relationship
	err := r.db.Where("user_id = ? AND target_id = ?", userId, targetId).First(&relationship).Error
	if err != nil {
		return nil, err
	}
...
func (r *RelationshipRepository) ManyByUserId(userId int) ([]*schemas.Relationship, error) {
	var relationships []*schemas.Relationship
	err := r.db.Where("user_id = ?", userId).Find(&relationships).Error
	return relationships, err
}

func (r *RelationshipRepository) ManyByTargetId(targetId int) ([]*schemas.Relationship, error) {
	var relationships []*schemas.Relationship
	err := r.db.Where("target_id = ?", targetId).Find(&relationships).Error
	return relationships, err
}
